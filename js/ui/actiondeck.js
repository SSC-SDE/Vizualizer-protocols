// Action deck — the role-play control surface, shared by both "be the client" and
// "run the server" directors. Shows every dispatchable action (WHAT), a target
// picker (WHO) and the four OSI lanes (LAYER); arming all three lights DISPATCH,
// which hands the triple to whichever director is active. Also owns the mission
// picker (both roles) and the recap.

import { KIND_COLOR } from '../sim/engine.js';
import { LAYER_OPTIONS } from '../app/roleplay.js';
import { MISSIONS, MISSION_ORDER, SERVER_MISSIONS, SERVER_MISSION_ORDER } from '../app/missions.js';
...
export class ActionDeck {
  constructor(directors) {
    this.dirs = directors;       // { client, server }
    this.rp = null;              // active director
    ...
    for (const d of Object.values(this.dirs)) {
      d.onState = () => this.render();
      d.onComplete = (stats) => this.showRecap(stats);
      d.onReject = (hint) => this._flash(hint, 'bad');
      d.onExit = () => this._hideAll();
    }

    this._buildPicker();
    this._buildDeckSkeleton();
  }

  // ---------------------------------------------------------------- toggle / overlays

  get active() { return Object.values(this.dirs).some((d) => d.active); }

  toggle() {
    const live = Object.values(this.dirs).find((d) => d.active);
    if (live) { live.exit(); return; }
    this._showPicker();
  }
  ...
  pickHost(host) {
    if (!this.rp) return;
    if (this.rp.role === 'server') {
      if (host?.kind === 'client') { this.sel.dst = 'requester'; this.render(); return; }
    }
  ...
  _buildPicker() {
    const cardsFor = (order, missions, role) => order.map((key) => {
      const m = missions[key];
      return `<button class="rp-card" data-role="${role}" data-mission="${key}">
    ...
    this.picker.innerHTML = `
      <div class="rp-modal rp-modal-wide">
        <div class="panel-title">🎭 ROLE-PLAY <span class="dim">// pick a side, then a mission</span></div>
        <div class="rp-group-t">🙋 BE THE CLIENT</div>
        <div class="rp-cards">${cardsFor(MISSION_ORDER, MISSIONS, 'client')}</div>
        <div class="rp-group-t">🖥 RUN THE SERVER</div>
        <div class="rp-cards">${cardsFor(SERVER_MISSION_ORDER, SERVER_MISSIONS, 'server')}</div>
        <div class="rp-foot"><span class="dim">You dispatch every packet by hand. Wrong picks teach — try them.</span>
          <button id="rp-picker-cancel">cancel</button></div>
      </div>`;
    this.picker.querySelectorAll('[data-mission]').forEach((b) => {
      b.onclick = () => { this.picker.hidden = true; this._start(b.dataset.role, b.dataset.mission); };
    });
    this.picker.querySelector('#rp-picker-cancel').onclick = () => { this.picker.hidden = true; };
  }

  _start(role, missionKey) {
    this.rp = this.dirs[role];
    this.sel = { kind: null, dst: null, layer: null };
    this._buildPickers();                 // WHAT/WHO depend on the role
    this.deck.hidden = false;
    if (this.controlsEl) this.controlsEl.hidden = true;
    this.rp.enter(missionKey);
  }

  // ---------------------------------------------------------------- deck skeleton + pickers
  ...
  _buildDeckSkeleton() {
    this.deck.innerHTML = `
      <div class="rp-left">
        ...
        <div class="rp-pickrow"><span class="rp-lbl">WHAT</span><span class="rp-opts" id="rp-what"></span></div>
        <div class="rp-pickrow"><span class="rp-lbl">WHO</span><span class="rp-opts" id="rp-who"></span></div>
        <div class="rp-pickrow"><span class="rp-lbl">LAYER</span><span class="rp-opts" id="rp-layer"></span></div>
  ...
  _dispatch() {
    if (!this.rp || !this.sel.kind || !this.sel.dst || !this.sel.layer) return;
  ...
  _flash(text, cls) {
    if (this.feedbackEl) this.feedbackEl.innerHTML = `<span class="${cls}">${text}</span>`;
  }

  // ---------------------------------------------------------------- render

  render() {
    if (!this.rp || !this.rp.active) { this.deck.hidden = true; return; }
    this.deck.hidden = false;
    const s = this.rp.getState();

    this.deck.querySelector('#rp-title').textContent =
      (s.role === 'server' ? '🖥 ' : '🙋 ') + s.mission.title;
    ...
    this.deck.querySelector('#rp-score').textContent = `sent ${s.packets} · mistakes ${s.mistakes} · hints ${s.hints}`;
    ...
    cl.innerHTML = s.steps.map((step, i) => {
      ...
      const label = step.auto
        ? `<span class="rp-auto">⟳ ${step.label || step.auto}</span>`
...
function whoName(key) {
  return { broadcast: 'broadcast', router: 'CORE-RTR', dns: 'DNS-CORE', web: 'WEB-EDGE', media: 'MEDIA-RELAY', requester: 'requester' }[key] || key;
}
